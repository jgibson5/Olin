...
{
    Hashable *h1 = (Hashable *) ip;
    Hashable *h2 = (Hashable *) jp;
    int val1 = hash_int(h1->key);
    int val2 = hash_int(h2->key);
    int equal = val1 == val2;
    return equal;
}
...
{
    Hashable *h1 = (Hashable *) s1;
    Hashable *h2 = (Hashable *) s2;
    char *str1 = (void *) h1->key;
    char *str2 = (void *) h2->key;
...
void map_add(Map *map, Hashable *key, Value *value)
{
    ...
    int k = *hash % map->n;
    if (map->lists[k] == NULL) {
        map->lists[k] = make_node(key, value, NULL);
    } else {
        //Node *prev = map->lists[k];
        //map->lists[k] = make_node(key, value, prev);
        map->lists[k]->next = make_node(key, value, NULL);
    }
}
...
Value *map_lookup(Map *map, Hashable *key)
{
    int *hash = (int *) key->hash;
    int k = *hash % map->n;
    return list_lookup(map->lists[k]->next, key);
}
...
int main ()
{
    ...
    Hashable *hashable3 = make_hashable_int (3);
